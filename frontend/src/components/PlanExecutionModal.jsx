...
export default function PlanExecutionModal({ isOpen, onClose, planAnalysis, onSave }) {
  ...
  const totalInvested = items.reduce((sum, item) => sum + ((Number(item.purchasePrice) || 0) * (Number(item.shares) || 0)), 0);
  ...
  return (
    <div style={{
      ...
      <div className="card fade-up" style={{ width: '90%', maxWidth: 650, padding: 24, maxHeight: '90vh', overflowY: 'auto' }}>
        <h3 style={{ margin: '0 0 8px 0', fontSize: '1.2rem', fontWeight: 700, color: '#00e5ff', display: 'flex', alignItems: 'center', gap: 8 }}>
          🚀 Asistente de Ejecución de Plan
        </h3>
        <p style={{ margin: '0 0 20px 0', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
          Monto total del plan: <strong style={{ color: '#f1f5f9' }}>${totalTarget.toFixed(2)} USD</strong>
        </p>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {items.map((item, idx) => {
              const currentInvested = (Number(item.purchasePrice) || 0) * (Number(item.shares) || 0);
              const diff = currentInvested - item.targetAmount;
              const isClose = Math.abs(diff) < (item.targetAmount * 0.1); // 10% tolerance

              return (
                <div key={idx} style={{ padding: 16, background: 'rgba(255,255,255,0.03)', borderRadius: 12, border: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                    <strong style={{ fontSize: '1.05rem', color: '#f1f5f9' }}>{item.ticker}</strong>
                    <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                      Objetivo: <span style={{ color: '#00e5ff', fontWeight: 600 }}>${item.targetAmount.toFixed(2)}</span>
                    </span>
                  </div>
                  
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: 4 }}>Precio de Apertura/Compra</label>
                      <input 
                        type="number" 
                        className="input" 
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: 4 }}>Volumen / Acciones</label>
                      <input 
                        type="number" 
                        className="input" 
                  ...
                  <div style={{ marginTop: 10, fontSize: '0.8rem', textAlign: 'right', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 8 }}>
                    Invertido: <strong style={{ color: currentInvested > 0 ? (isClose ? '#4ade80' : '#f59e0b') : 'var(--text-muted)' }}>${currentInvested.toFixed(2)}</strong>
                    {currentInvested > 0 && (
        ...
        <div style={{ padding: 16, background: 'rgba(0,0,0,0.3)', borderRadius: 12, marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Resumen de Ejecución:</span>
          <strong style={{ fontSize: '1.2rem', color: Math.abs(totalInvested - totalTarget) < (totalTarget * 0.1) ? '#4ade80' : '#f1f5f9' }}>
